...
|-------------|----------------|-------------:|-----------:|----------------:|-------------:|----------:|-----------------:|-----------------------:|-------:|
| QLoRA r=16  | 1× A100 SXM    |     35.5 GB  |    3.09 h  |  4.50 samples/s |       97.4 % |   380 W   |           0.376  |               7.77e17  |  $5.56 |
| LoRA r=16   | 1× A100 SXM    |     64.8 GB  |    1.43 h  |  9.72 samples/s |       94.3 % |   380 W   |           0.374  |               7.77e17  |  $2.57 |
| Full FT     | 4× A100 SXM    |     73.9 GB  |    0.97 h  | 14.27 samples/s |       86.6 % |   315 W   |        **0.269** |               1.58e17  |  $7.01 |
...
Peak GPU mem for Full FT is per-GPU after FSDP sharding (live state across 4 GPUs sums to ~296 GB — physically impossible on a single device, which is why FSDP is required). All runs use **A100 80GB SXM** (RunPod); cost is computed at ~$1.80/h per GPU.
...
- **LoRA bf16** — the obvious winner. Best quality, lowest cost (~$2.57 on 1× A100 SXM), identical training dynamics to QLoRA. Pick this if you have ≥80 GB VRAM.
...
- **Hardware difference for Full FT.** Full FT runs on 4× A100 80GB SXM (FSDP); QLoRA/LoRA on 1× A100 80GB SXM. Same GPU model, only the count differs — Full FT couldn't physically fit on a single GPU. This affects wall-time but not the eval scores.
...
# Phase 2: QLoRA training + eval  (~3h, 1× A100 80GB SXM)
bash scripts/run_qlora.sh

# Phase 3: LoRA bf16 training + eval  (~1.5h, 1× A100 80GB SXM)
